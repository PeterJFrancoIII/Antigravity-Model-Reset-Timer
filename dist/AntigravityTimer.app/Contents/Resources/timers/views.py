...
def bulk_update_account(request, account_id):
    """Updates timers and percentages for all models from a single form submission."""
    if request.method == 'POST':
        account = get_object_or_404(AntigravityAccount, id=account_id)
        
        for model in ['flash', 'pro', 'opus']:
            hours_raw = request.POST.get(f'{model}_hours', '')
            minutes_raw = request.POST.get(f'{model}_minutes', '')
            percent_raw = request.POST.get(f'{model}_percent', '')
            ...
            if hours_raw or minutes_raw:
                ...
                if hours > 0 or minutes > 0:
                    future_time = timezone.now() + timedelta(hours=hours, minutes=minutes)
                    if model == 'flash': account.gemini_flash_reset = future_time
                    elif model == 'pro': account.gemini_pro_reset = future_time
                    elif model == 'opus': account.opus_sonnet_reset = future_time
            
            if percent_raw:
                percent = int(percent_raw)
                if model == 'flash': account.gemini_flash_percent = percent
                elif model == 'pro': account.gemini_pro_percent = percent
                elif model == 'opus': account.opus_sonnet_percent = percent
